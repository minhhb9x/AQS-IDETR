...
if __name__ == '__main__':
    ...
    im_data = transforms(im_pil)[None]  # shape (1,3,640,640)

    # scale_factor = resized_size / orig_size
    scale_y = 640.0 / h
    scale_x = 640.0 / w

    # Build blob khớp input names trong TensorRT engine
    blob = {
        'image': im_data.to(args.device).contiguous(),               # (1,3,640,640)
        'im_shape': torch.tensor([[h, w]], dtype=torch.float32,      # (1,2)
                                 device=args.device),
        'scale_factor': torch.tensor([[scale_y, scale_x]], dtype=torch.float32,
                                      device=args.device),           # (1,2)
    }
