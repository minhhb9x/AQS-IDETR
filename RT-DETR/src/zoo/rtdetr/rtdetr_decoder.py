...
class TransformerDecoder(nn.Module):
    ...
    def forward(self,
        ...
        for i, layer in enumerate(self.layers):
            ...
            # m_v, _ = dec_out_logiti.max(-1)
            # _, sorted_idx = torch.sort(m_v, dim=1, descending=True)
            # batch_idx = torch.arange(output.size(0), device=output.device).unsqueeze(-1)
            # output = output[batch_idx, sorted_idx]     # [bs, num_query, hidden_dim]
            # ref_points_detach = ref_points_detach[batch_idx, sorted_idx]
            # inter_ref_bbox = inter_ref_bbox[batch_idx, sorted_idx]
            ...
            if self.infer_adapt:
                ...
                if i == len(self.layers) - 1:
                    ...
                    # self.n_layer_query[i] += max(sub_seq_len)
                    pass
                else:
