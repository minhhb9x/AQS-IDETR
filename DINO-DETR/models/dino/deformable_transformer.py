...
class DeformableTransformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8, 
        ...
        self.decoder = TransformerDecoder(decoder_layer, num_decoder_layers, decoder_norm,
                                        ...
                                        beta=beta,
                                        gamma=gamma,
                                        num_fixed_queries=num_fixed_queries
                                        )
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, decoder_layer, num_layers, norm=None, 
        ...
        if num_layers > 0:
            ...
            self.layers = []
        self.n_layer_query = [0 for _ in range(num_layers)]
        self.num_fixed_queries = num_fixed_queries
    ...
    def forward(self, tgt, memory,
        ...
        #tt = 200
        if not self.infer_adapt: # check fixed queries
            output = output[:self.num_fixed_queries]
            reference_points = reference_points[:self.num_fixed_queries]
        for layer_id, layer in enumerate(self.layers):
            if self.infer_adapt:
                seqs_o = seqs.clone()
            ...
            if self.infer_adapt:
                ...
                if layer_id < len(self.layers) - 1:
                    ...
                    self.n_layer_query[layer_id] += float(max(seqs))
                else:
                    tt = max(seqs)
                    self.total_last_layer += tt
                    self.n_call += 1
                    self.n_layer_query[layer_id] += float(tt)
            # iter update
...
def build_deformable_transformer(args):
    ...
    return DeformableTransformer(
        ...
        beta = args.beta,
        gamma = args.gamma,
        num_fixed_queries = args.num_fixed_queries
    )
